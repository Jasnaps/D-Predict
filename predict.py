...
DF_train= pd.read_csv('dataset\Training.csv', index_col='prognosis')
...
DF_test= pd.read_csv('dataset\Testing.csv', index_col='prognosis')
...
def predictor(final_symptoms):
    ...
    for final_symptom in final_symptoms:
        l2[vocab.index(final_symptom)] = 1
    y_test_in =[l2]
    predict = gnb.predict(y_test_in)
    predicted=predict[0]
    
    flag='no'
    for code in range(0,len(disease)):
        if(predicted == code):
            flag='yes'
            break
    if(flag== 'yes'):
            predn.append(disease[code])
    return predn
